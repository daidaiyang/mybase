package com.example.ygslibrary.subscribers;
...
import com.example.ygslibrary.http.cookie.CookieResulte;
import com.example.ygslibrary.listener.HttpOnNextListener;
import com.example.ygslibrary.utils.AppUtil;
import com.example.ygslibrary.utils.CookieDbUtil;
        ...
        if (api.isCache()){
            Observable.just(api.getUrl()).subscribe(new Subscriber<String>() {
                ...
                public void onNext(String s) {
                    ...
                    if (cookieResulte == null) {//没有缓存
                        throw new HttpTimeException("网络错误");
                    }
                    long time = (System.currentTimeMillis() - cookieResulte.getTime()) / 1000;
                    if (time < api.getCookieNoNetWorkTime()) {
                        if (mSubscriberOnNextListener.get() != null) {
                            //获取缓存成功，切换到onCacheNext
                            mSubscriberOnNextListener.get().onCacheNext(cookieResulte.getResulte());
                        }
                    } else {
